...
describe('Table Component Test Suit', () => {
  ...
  it('renders table headers correctly', () => {
    render(<Table data={data} headers={headers} renderCell={() => null} ariaLabel="Table" />);
  ...
  it('renders table rows correctly', () => {
    render(<Table data={data} headers={headers} renderCell={item => <tr key={item.id}><td>{item.id}</td><td>{item.marca}</td><td>{item.plate}</td></tr>} ariaLabel="Table" />);
    data.forEach(item => {
